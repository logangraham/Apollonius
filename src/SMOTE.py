...
class SMOTE(object):
    ...
    def _infer_cols(self, X):
        if isinstance(X, pd.core.frame.DataFrame):
            X = X.values()
        X = pd.DataFrame(X)
        cat_cols = list(X.columns[X.nunique() <
                                  self.categorical_nunique_thres])
        non_cat_cols = [i for i in X.columns if i not in cat_cols]
        self.column_order = np.argsort(non_cat_cols + cat_cols)
